...
public class StorageCrc64Calculator {
    ...
    private static long mulPolyUnrolled(long a, long b) {
        ...
        final long[] vs = { bw - 2, bw - 1 };
        long[] vb = { (b >>> 1) ^ vt[(int) ((b & 1) << 1)], b }; // Unsigned right shift
        long[] vr = { 0, 0 };

        for (long i = 0; i < bw; i += 2) {
            for (int j = 0; j < 2; ++j) {
                vr[j] ^= vb[j] * ((a >>> vs[j]) & 1); // Unsigned right shift
                vb[j] = (vb[j] >>> 2) ^ vt[(int) (vb[j] & 3)]; // Unsigned right shift
            }
            a <<= 2; // Shift remains the same as a is multiplied by 4
        }
